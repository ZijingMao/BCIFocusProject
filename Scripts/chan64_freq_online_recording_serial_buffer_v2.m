...
for row = 1:8
    for col = 1:8
        axes_h = subplot(8,8,(row-1)*8+col, 'Parent', CCA_figure_handle);
        bar_h = bar(freq_data((row-1)*8+col, :),...
            'Parent', axes_h);
        bar_handles((row-1)*8+col) = bar_h;
        set(axes_h, ...
            'XTickLabel',xtick_lbl, ...
            'XTick',xtick);
        xlim(axes_h,[0 length(freq_band)+1]);
        ylim(axes_h, [-50 50]);
        title(processed_EEG.chanlocs((row-1)*8+col).labels);
    end
end
...
while buffer_op_status
    ...
    for row = 1:8
        for col = 1:8
            set(bar_handles((row-1)*8+col), 'Ydata', freq_data((row-1)*8+col, :));
        end
    end
