function [op_status, EEG] = ssvepLiveProcessingAndDetection_v2(...
    EEG_epoch, Fs, pb_filter, chanlocs_pos)
...
%
% Devloped by the Dr. Yufei Huang EEG Laboratory, University of Texas at
% San Antonio, (UTSA), Nov. 2014.
% Developer: Mauricio Merino. email: kxl082@my.utsa.edu
...
% Referencing based on the EX1 and EX2 electrodes
    ref_mean = mean([EEG.data(end, :), EEG.data(end-1, :)]);
    EEG.data = EEG.data - ref_mean;
    ...
    % Remove first channel (flat noise) and EX's channels
    nchan = size(EEG.data, 1);
    EEG.data = EEG.data(1:nchan-2, :);   % Remove EX1, EX2 channels
    EEG.nbchan = size(EEG.data, 1);
    EEG.chanlocs = sample_channels(chanlocs_pos);
    EEG.pnts = size(EEG.data, 2);
